<x-admin-layout>
    <x-slot:title>Modifier le Projet</x-slot>
    <x-slot:header>Éditer : {{ $project->name }}</x-slot>
        ...
        <form action="{{ route('admin.projects.update', $project) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="goal" id="goal" value="{{ old('goal', $project->goal) }}"
                        class="w-full rounded-lg border-gray-300 shadow-sm focus:border-foot-violet focus:ring-foot-violet sm:text-sm">
            ...
            <div>
                ...
                @if($project->image_url)
                    <div class="mt-2">
                        <img src="{{ $project->image_url }}" alt="Preview" class="h-20 w-32 object-cover rounded shadow-sm">
                    </div>
                @endif
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="6" required
                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-foot-violet focus:ring-foot-violet sm:text-sm">{{ old('description', $project->description) }}</textarea>

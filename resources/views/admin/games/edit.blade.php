<x-admin-layout>
    <x-slot:title>Modifier le Match</x-slot>
    <x-slot:header>Éditer le match contre {{ $game->opponent }}</x-slot>
        ...
        <form action="{{ route('admin.games.update', $game) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="match_date" id="match_date" value="{{ old('match_date', \Carbon\Carbon::parse($game->match_date)->format('Y-m-d\TH:i')) }}" required
                        class="w-full rounded-lg border-gray-300 shadow-sm focus:border-foot-violet focus:ring-foot-violet sm:text-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="home_score" class="block text-sm font-bold text-gray-700 mb-1">Score Nassira FC</label>
                    <input type="number" name="home_score" id="home_score" value="{{ old('home_score', $game->home_score) }}" min="0"
                        class="w-full rounded-lg border-gray-300 shadow-sm focus:border-foot-violet focus:ring-foot-violet sm:text-sm">
                    @error('home_score') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="away_score" class="block text-sm font-bold text-gray-700 mb-1">Score Adversaire</label>
                    <input type="number" name="away_score" id="away_score" value="{{ old('away_score', $game->away_score) }}" min="0"
                        class="w-full rounded-lg border-gray-300 shadow-sm focus:border-foot-violet focus:ring-foot-violet sm:text-sm">

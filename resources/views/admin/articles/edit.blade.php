<x-admin-layout>
    <x-slot:title>Modifier l'Article</x-slot>
    <x-slot:header>Éditer : {{ $article->title }}</x-slot>
        ...
        <form action="{{ route('admin.articles.update', $article) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                @if($article->image_url)
                    <div class="mt-2">
                        <img src="{{ $article->image_url }}" alt="Preview" class="h-20 w-32 object-cover rounded shadow-sm">
                    </div>
                @endif
            </div>
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="10" required
                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-foot-violet focus:ring-foot-violet sm:text-sm">{{ old('content', $article->content) }}</textarea>
                @error('content') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="published_at" class="block text-sm font-bold text-gray-700 mb-1">Date de publication</label>
                <input type="date" name="published_at" id="published_at" value="{{ old('published_at', $article->published_at ? $article->published_at->format('Y-m-d') : '') }}"
                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-foot-violet focus:ring-foot-violet sm:text-sm">
